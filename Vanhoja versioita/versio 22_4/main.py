...
########################################
    ...
    def textfield_test(self):
        header = "Kubios Result"
        print(header)
        ...
        while True:
            ...
            while not enc1.fifo.empty():
                try:
                    enc_input += enc1.fifo.get()
                except:
                    pass

            scroll_index += enc_input

            if scroll_index < 0:
                scroll_index = 0
            elif scroll_index > len(lines) - max_lines_on_screen:
                scroll_index = len(lines) - max_lines_on_screen

            oled.fill(0)
            # Print text
            for i in range(max_lines_on_screen):
                if scroll_index + i < len(lines):
                    oled.text(lines[scroll_index + i], 0, i * 9, 1)
            oled.show()
